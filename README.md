...
- [x] **全局搜索** - 跨分类搜索，支持名称、URL、备注模糊匹配
- [x] **局部搜索** - 当前分类内搜索过滤
- [x] **双视图模式** - 列表视图（表格）与网格视图（卡片），偏好持久化
- [x] **分页功能** - 可配置每页条数（10/20/50/100），分页导航
...
| `GET` | `/api/items` | 获取列表（支持分页、搜索、分类筛选） | 否 |
| `POST` | `/api/items` | 新增条目 | 是 |
...
```bash
wrangler d1 execute recohub-db --file=./db/schema.sql
wrangler d1 execute recohub-db --file=./db/seed.sql
```
