...
- [x] **链接健康检查** - 后台自动定期检测所有条目 URL 可用性（HEAD + GET 降级，15s 超时），连续 3 次失败标记为"已失效"；设置面板中可查看失效链接、重置状态、调整检查间隔（1h/6h/12h/24h）
